...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub(super) fn transfer_claim(
			...
			// Store the proof with the new owner and block number.
			Proofs::<T>::insert(&proof, (&reciver, current_block));


			// Emit an event that the clain was transfered.
			Self::deposit_event(Event::ClaimTransfered(sender, proof, reciver));
